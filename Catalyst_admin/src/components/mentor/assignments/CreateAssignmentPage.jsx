...
function StepInfo({ data, onChange, students }) {
  ...
  return (
    <div className="max-w-2xl mx-auto space-y-7">
      ...
      {/* Assign to students */}
      {students.length > 0 && (
        <Field label="Assign to Students" hint="Students will see this assignment in their portal">
          <div className="grid grid-cols-2 gap-2">
            {students.map((s) => {
              const selected = (data.assignedTo || []).includes(s.id);
              return (
                <button
                  key={s.id}
                  type="button"
                ...
                >
                  <div
                    className={`w-8 h-8 rounded-full flex items-center justify-center text-xs font-extrabold shrink-0 ${
                      selected ? 'bg-indigo-500 text-white' : 'bg-gray-200 text-gray-600'
                    }`}
                  >
                    {s.avatar || s.name.slice(0, 2).toUpperCase()}
                  </div>
                  <div className="min-w-0 flex-1">
                    <p className={`text-[13px] font-semibold truncate ${selected ? 'text-indigo-700' : 'text-gray-800'}`}>
                      {s.name}
                    </p>
                    <p className="text-[11px] text-gray-400 truncate">{s.course || s.batch || ''}</p>
                  </div>
                  {selected && <span className="text-indigo-500 text-sm shrink-0">✓</span>}
                </button>
              );
            })}
          </div>
